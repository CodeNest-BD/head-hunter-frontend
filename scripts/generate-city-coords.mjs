 ...
 *  - us_city_coordinates.csv     — 29.9k US cities with lat/lng + county
 *    (public-domain Census/GNIS facts, via kelvins/US-Cities-Database)
 *
 ...
 * coordinated city in the county (so a CDP like Candler-McAfee still lands in
 * the right county — DeKalb, just east of Atlanta — not at the state centre).
 * A city we can resolve neither way is omitted and falls back to the state
 * centroid at runtime.
 *
...
const coordsPath = join(
  root,
  "requirements/usa-cities/us_city_coordinates.csv",
);
...
const c = {
  code: coords.header.indexOf("STATE_CODE"),
  city: coords.header.indexOf("CITY"),
  county: coords.header.indexOf("COUNTY"),
  lat: coords.header.indexOf("LATITUDE"),
  lng: coords.header.indexOf("LONGITUDE"),
};
